...
int product_tree()
{
  ...
  double input_start = now();
  double output_start, output_sum = 0, input_sum = 0;
  input_bin_array(&v, INPUT_FN);
  input_sum += now() - input_start;
  fprintf(stderr, "input file: %0.3fs\n", input_sum);
  int level = 0;
  while (v.count > 1)
  {
    // double start = now();
    ...
    output_sum += now() - output_start;
    free_vec(&v);
    v = w;
    level++;
    //   fprintf(stderr, "%0.3fs\n", now()-start);
  }
  fprintf(stderr, "output file: %0.3fs\n", output_sum);
  free_vec(&v);
  fprintf(stderr, "product tree took %0.3fs\n", now() - tstart - input_sum - output_sum);
  return level;
}
...
void remainder_tree(int level)
{
  ...
  P = v;
  */
  double input_start, output_start;
  ...
  output_sum += now() - output_start;
  fprintf(stderr, "%0.3fs\n", now() - start);
  fprintf(stderr, "input file: %0.9fs\n", input_sum);
  fprintf(stderr, "output file: %0.9fs\n", output_sum);

  free_vec(&w);
  free_vec(&v);
  free_vec(&P);
  fprintf(stderr, "remainder tree took %0.3fs\n", now() - start - input_sum - output_sum);
}
...
int main(int argc, char *argv[])
{
  ...
  fprintf(stderr, "product tree run took %0.3fs\n", now() - start);
  start = now();
  remainder_tree(level - 1);
  fprintf(stderr, "remainder tree run took %0.3fs\n", now() - start);
  start = now();
  emit_results();
  fprintf(stderr, "emit result run took %0.3fs\n", now() - start);
  fprintf(stderr, "all time  run took %0.3fs\n", now() - all_time);
  return 0;
}
